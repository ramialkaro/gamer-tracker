module.exports=function(server,table,templateBuilder,hasErrors){
    ...
    return {
        ...
        getById(id,cb){
            ...
            else server.paramQuery("SELECT * FROM ?? WHERE id=?",[table,id],({error,data}) => {
        ...
        update(values,cb){
            ...
            if (notValid) cb(notValid); 
                ...
                else server.paramQuery("UPDATE "+table+" SET ? WHERE id="+id,value,({error})=>{
        ...
        delete(id,cb){
            ...
            else server.query("DELETE FROM "+table+" WHERE id="+id,({error,data}) => {
